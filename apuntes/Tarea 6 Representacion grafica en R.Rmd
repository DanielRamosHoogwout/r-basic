...
plot(f, main = "Una parábola", 
     xlab = (expression("x")), 
     ylab = (expression(y=x^2-3*x+30)), 
     xlim=c(-15,15))

#Añado una curva x^2 para comparar
...
#Es igual de valida que en el caso anterior, no hay diferencia.
```
...
legend(x=13,y=10, legend =c(expression(3*x),expression(-3*x)), col = c("blue","green"), lwd = 2, lty = c("solid","solid"))
```
...
```{r Ej5}
#abline(h=0, col = "red", lwd=5, add = TRUE)
```
